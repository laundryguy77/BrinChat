...
async def full_unlock(
    ...
    - Tier 1 (adult_mode via passcode in Settings) must be enabled first
...
async def generate_avatar_options(
    ...
    # Avatar generation via MCP has been removed in BrinChat refactor
    # This feature would require an image generation backend to be configured
    logger.warning("Avatar generation requested but MCP has been removed")
    return AvatarGenerateResponse(
        success=False,
        images=[],
        error="Avatar generation is not available. The image generation backend has been removed in this version."
    )

    # Placeholder for future implementation without MCP
    images = []
    if False:  # Dead code - kept for reference
        try:
            pass  # Future: implement with built-in image backend
...
async def get_full_unlock_status(
    ...
    Returns:
    - adult_mode: Tier 1 status (passcode via Settings)
