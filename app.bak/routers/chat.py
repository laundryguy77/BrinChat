...
def extract_tts_audio(content: str) -> tuple[str, Optional[str]]:
    ...
    Returns:
        tuple: (cleaned_content, audio_path_or_None)
    """
    if not content:
        return content, None
    ...
    # Strip the MEDIA: line from content, replace with single newline to avoid merging lines
...
async def chat(request: Request, user: UserResponse = Depends(require_auth)):
    ...
    async def event_generator():
        ...
        tts_semaphore = None
        tts_tasks = []  # List of asyncio.Tasks for pending TTS
        tts_index = 0
        tts_voice = "alloy"
        tts_speed = 1.0
        ...
        try:
                        ...
                        if voice_response and sentence_buffer:
                            ...
                            for task in completed:
                                ...
                                try:
                                    result = task.result()
                                    if result is None:
                                        continue  # Empty sentence after cleaning
            ...
            if voice_response and sentence_buffer:
                ...
                # Wait for all remaining TTS tasks and yield their results
                if tts_tasks:
                    ...
                    for result in results:
                        if isinstance(result, Exception):
                            logger.error(f"[StreamTTS] TTS task failed: {result}")
                            continue
                        if result is None:
                            continue  # Empty sentence after cleaning
                        seq_idx, audio_url = result
                ...
                # Signal frontend that all TTS chunks have been sent
                yield {
                    "event": "tts_done",
                    "data": json.dumps({"tts_done": True})
                }
                logger.info(f"[StreamTTS] All {tts_index} sentence(s) dispatched")
            ...
            if tool_calls:
                ...
                if followup_content:
                    ...
                    from app.config import EXTRACTION_ENABLED
                ...
                if collected_content:
                    ...
                    cleaned_content, memories_extracted = strip_memory_tags(cleaned_content)
                    
                    # If content was modified (MEDIA tags or memory tags), send replacement
                    ...
                    # Skip if streaming TTS already handled voice for this response
                    if tts_audio_path and not voice_response:
                        try:
                            import os
                            if os.path.isfile(tts_audio_path):
                                # Build a relative URL that the frontend can fetch
                                # Path is like /tmp/tts-XXXXX/voice-YYYY.mp3
                                # URL becomes /api/voice/media/tts-XXXXX/voice-YYYY.mp3
                                ...
                                logger.info(f"[TTS] Sent audio URL to frontend: {audio_url}")
                            else:
                        ...
                        logger.info(f"[TTS] Skipping MEDIA: audio — streaming TTS already active")
                    ...
                    from app.config import EXTRACTION_ENABLED
                    if EXTRACTION_ENABLED and cleaned_content:
                        try:
                            from app.services.async_extractor import queue_extraction
                            await queue_extraction(
                                user_id=user.id,
